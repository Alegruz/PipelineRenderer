...
namespace pr
{
    void ClearDepth(ID3D12GraphicsCommandList2* pCommandList, D3D12_CPU_DESCRIPTOR_HANDLE dsv, FLOAT depth) noexcept
    {
        pCommandList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH, depth, 0, 0, nullptr);
    }

    void ClearDepth(ID3D12GraphicsCommandList2* pCommandList, D3D12_CPU_DESCRIPTOR_HANDLE dsv) noexcept
    {
        ClearDepth(pCommandList, dsv, 1.0f);
    }

    void ClearRtv(_In_ ID3D12GraphicsCommandList2* pCommandList, _In_ D3D12_CPU_DESCRIPTOR_HANDLE rtv, _In_ const FLOAT* pClearColor) noexcept
    {
        pCommandList->ClearRenderTargetView(rtv, pClearColor, 0, nullptr);
    }
    ...
    void TransitResource(ID3D12GraphicsCommandList2* pCommandList, ID3D12Resource* pResource, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState) noexcept
    {
    ...
    ) noexcept
    {
        ...
        CD3DX12_RESOURCE_DESC destBufferDesc = CD3DX12_RESOURCE_DESC::Buffer(bufferSize, flags);
        ...
        if (pBufferData)
        {
            ...
            CD3DX12_RESOURCE_DESC intermediateBufferDesc = CD3DX12_RESOURCE_DESC::Buffer(bufferSize);
